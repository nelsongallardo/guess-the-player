...
- Daily and Unlimited keep independent per-tab clocks. A mode's clock starts when that mode opens, survives refresh and mode switching in the same tab, resets for its next player, and is not restored from Daily's long-lived progress after the tab closes. Switching modes is immediate and does not show a confirmation dialog.
