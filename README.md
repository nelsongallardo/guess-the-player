...
- A correct answer earns up to 100 points, reduced by 20% per hint and by answer speed: full value inside a 2-second grace window, decaying to a 25% floor by 12 seconds — short enough that an outside lookup can't out-score a fast, honest guess (see [ADR 0009](docs/adr/0009-speed-decay-anti-lookup-tightening.md)). A loss earns zero. Guest elapsed time is an in-memory round clock; ranked time uses the persisted server start and includes time away. Reloading cannot restart ranked timing. A minimal `#speed-meter` strip along the top of the career panel shows this curve live and freezes once the round resolves; clicking it explains the elapsed time and points.
