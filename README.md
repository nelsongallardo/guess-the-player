...
- 120 playable careers: 60 representing European national teams and 60 representing South American national teams — this number grows with each research batch; [research and audit](DATA_AUDIT.md) document every expansion and its evidence, and the footer of the app itself always shows the current total.
...
- Wrong answers draw from playable players plus a bank of researched wrong-answer-only profiles (59 as of this writing, and growing — see `research/verified-distractors.json` for the current count). Starting domestic football system takes precedence, then contemporary overlap with debuts at most eight years apart, then career similarity with bounded noise. This does not add playable rounds. See [ADR 0003](docs/adr/0003-researched-contemporary-rivals.md).
