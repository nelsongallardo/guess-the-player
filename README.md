...
The bare Play URL opens the **Daily challenge** (*Desafío diario*), branded **Daily Rabona** on its own card and share text. Each UTC challenge has the same three frozen players for everyone, played sequentially; each player keeps the normal three guesses, three hints, timer and points. The Daily completes, updates its device-local streak once, and enables its spoiler-free aggregate share only after player three. Choose **Unlimited play** (*Juego sin límite*) for the existing guest/ranked game; `?unlimited=1` preserves that explicit choice across reload and OAuth, while legacy `?daily=1` links canonicalize to the bare Daily URL (with a valid `lang` when present). The two mode buttons name themselves as complete noun phrases and each carries two plain fact lines, so the difference is legible before the choice rather than after it.

For guests, Daily is local and non-ranked. Its answers and progress use `localStorage['derabona.daily.v1']`, while its active-player clock uses the separate per-tab `sessionStorage['derabona.daily-clock.v1']`; it never changes Unlimited guest or account progress. Its document is schema/schedule v2. The briefly shipped one-player v1 document is deliberately reset under that same key instead of attempting a speculative partial migration; frozen v1 payload and schedule literals remain unchanged and v2 deterministically groups three consecutive descriptors per UTC date.

Signed in, Daily is server-authoritative like ranked play, and its points count into the same account/leaderboard totals as career results ([ADR 0021](docs/adr/0021-daily-rabona-server-authoritative-for-accounts.md)). The server derives today's three players and options from the same frozen schedule the offline guest artifact ships, tracks each round's guesses/hints/points, and keeps a per-account Daily streak (current and best), rankable on its own streak leaderboard. Signing in mid-day never imports that day's already-started guest attempt - it stays local and unranked, and Daily's three players restart fresh under the account, matching Unlimited's existing guest-score-does-not-transfer rule.
...
- Daily and Unlimited keep independent per-tab clocks. A mode's clock starts when that mode opens, survives refresh and mode switching in the same tab, resets for its next player, and starts fresh after normal tab closure. Unlimited also rejects restored history/page-cache clocks and clocks cloned into a newly opened tab, without treating ordinary same-tab navigation as a reset, and never trusts a stored clock more than six hours old regardless of those signals. Switching modes is immediate and does not show a confirmation dialog.
...
Keyboard-operable controls, visible focus, text plus color feedback, live announcements and reduced-motion support are part of the contract. Careers always read in the same plain order — left to right, then down — at every width, as one flat wrapping grid: four columns at 800px and below ([ADR 0016](docs/adr/0016-career-grid-at-every-width.md)), six columns above it ([ADR 0021](docs/adr/0021-wrapping-desktop-career-grid.md)). No career needs horizontal scrolling at any width, and exceptionally long ones compact their cards on desktop so the answer options stay near the fold. [TESTING.md](TESTING.md) records actual browser coverage and historical limitations rather than promising every browser or viewport is verified.
