...
Keyboard-operable controls, visible focus, text plus color feedback, live announcements and reduced-motion support are part of the contract. Careers always read in the same plain order — left to right, then down — at every width, as one flat wrapping grid: four columns at 800px and below ([ADR 0016](docs/adr/0016-career-grid-at-every-width.md)), six columns above it ([ADR 0022](docs/adr/0022-wrapping-desktop-career-grid.md)). No career needs horizontal scrolling at any width, and exceptionally long ones compact their cards on desktop so the answer options stay near the fold. [TESTING.md](TESTING.md) records actual browser coverage and historical limitations rather than promising every browser or viewport is verified.
