...
- The lifetime score is displayed in the masthead itself (a `.score-badge`), not the hero `.stats` box — it's global to the whole device, not scoped to whichever competition is on screen. The hero `.stats` box now holds only the win streak.
