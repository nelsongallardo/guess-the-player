...
let edited = false, wrongCard = false, extraUrl = false, different = false;
...
<div data-testid="tweetText">${different ? 'Entradas para un concierto, nueva fecha confirmada en noviembre.' : body}${edited ? ' EDITADO' : ''}${extraUrl ? ' https://example.org/extra' : ''}</div>
...
try {
 ...
 for(const variant of ['edited','wrongCard','extraUrl']) {
  ...
  await publisher.inspectSource(job);await publisher.prepareReply(job);
  assert.equal(await page.locator('[role="dialog"] [contenteditable]').innerText(),job.replyText);
 }
 console.log('PASS small edit, changed attachment and extra URL accepted by the owner-requested relaxed check');
 different=true;
 await assert.rejects(()=>publisher.inspectSource(job),e=>e.code==='source_changed');
 assert.equal(await page.locator('[role="dialog"]').count(),0);
 different=false;
 console.log('PASS clearly different primary text still blocked before composing');
 ...
 globalThis.fetch=async()=>{throw new Error('source check must not depend on a short-link network lookup');};
 try {
  const legacy={...job,sourceLinks:[]};
  const publication=await publisher.publish(legacy);
  assert.equal(publication.parentId,job.sourceId);
 } finally {globalThis.fetch=originalFetch;}
 console.log('PASS legacy URL rendering works through submission/public-parent verification without redirect requests');
