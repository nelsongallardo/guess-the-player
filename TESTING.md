...
## Abandoned ranked round staleness — 20 September 2026

- Third owner report of "the timer shows a huge number even though I just opened the tab", this time **73:25**. The two earlier fixes could not explain it: 73 minutes is *below* both existing six-hour ceilings. The screenshot's decisive details were that the account was **signed in** (451 points, account icon) in **Unlimited play** with **3 attempts left** — a server-backed clock on an untouched round.
- Root cause is structural, not display: `ranked_private.rounds.started_at` defaults to `clock_timestamp()` at **row creation**; `RankedUI.sync()` auto-starts a round on page load when none is open; and `public.ranked_game`'s `start` branch returns any existing `status='playing'` round. Landing on the site signed in therefore starts a scoring clock on a puzzle that may never be looked at, and returning later resurrects it. Because `ranked_private.points()` charges `clock_timestamp()-started_at` and the curve floors at 24s, the player was **locked to 25 points before seeing the puzzle** — a real scoring bug, not cosmetic.
- Fixed by `supabase/migrations/202609200001_abandoned_ranked_round_staleness.sql` ([ADR 0023](docs/adr/0023-abandoned-ranked-round-staleness.md)): an existing playing career round with zero guesses, zero hints and `started_at` older than 30 minutes is deleted and re-dealt with a fresh clock. Engaged rounds (any hint or guess) are never replaced, preserving the ADR 0009 anti-lookup boundary. Daily rounds are untouched.
- Real isolated-PostgreSQL verification: **29/30 pass**, including four new tests covering the replacement, the engaged-round exemption (checked for both hint-engagement and guess-engagement), normal resumption inside the window, and that abandoning writes no `results` row and so cannot farm players or consume them.
- The one remaining failure (`a correct daily answer scores via the shared points curve…`) was reproduced **identically against unmodified `origin/main`** on the same runtime before this branch's migration existed, so it is pre-existing and out of scope. No assertion was weakened or removed.
- Caught during testing: the migration was first based on `202609140006_ranked_ten_options_v2.sql`, which silently reverted the later daily RPC actions and regressed **ten** daily tests. Rebased on the current definition in `202609180001_daily_rabona_schema.sql`; all ten recovered. A `create or replace function` migration here must always be based on the newest definition of that function, not on the migration that introduced the branch being changed.
- Also corrected a duplicate ADR number introduced earlier this session: PR #21 added `0021-wrapping-desktop-career-grid.md` while `d75d5d7` landed `0021-daily-rabona-server-authoritative-for-accounts.md`. The career-grid ADR is renumbered to **0022** with all references updated; the daily ADR keeps 0021.
- The client-side `displayableElapsedMs` six-hour display cap is retained. It is now a redundant second layer for this path, and still covers the gap between deploying the frontend and applying the migration, which is a deliberately manual step.


## Elapsed-time display ceiling for server-backed clocks — 19 September 2026

- The owner reported Unlimited still showing "Time played 2251" on a freshly opened tab, after the six-hour guest-clock ceiling had already shipped. That ceiling (`restoredRoundClockStart`) only ever guarded the guest `sessionStorage` clock. `currentRoundClock()` routes signed-in play to `RankedUI.roundClock()` (and Daily to `DailyRankedUI.clock()`), both of which compute `elapsedMs` directly from a server `startedAt` that legitimately survives closing the tab for days - so an abandoned signed-in round reopened later rendered ~37.5 hours as `2251:xx`. 2251 minutes is well past the six-hour cap, which is what identified the path as a different one.
- Added `displayableElapsedMs`, one shared display-level ceiling applied inside `elapsedTimeFrame()` for **all three** clock sources; `renderElapsedTime()` hides `#elapsed-time` rather than rendering an implausible value. Display-only: scoring stays server-authoritative and the 25% floor is still reached at 24s.
- Real-Chromium reproduction and fix confirmation: driving the display path with a 2251-minute ranked clock returned `wouldHaveRendered:"2251:00"` from the formatter while the guarded frame returned `shown:null`; a 37-hour Daily clock was likewise blocked; a fresh ranked clock still rendered `0:18` and a live guest clock `0:04`.
- Local Node verification passed **130/130** across the elapsed-time, model, daily-ui, daily-challenge, account-boundaries, guest-session, points-FAQ, SEO, career-arrow-layout, social-preview, privacy and leaderboard suites; `tests/source-check.py`, `scripts/generate-daily-schedule.mjs --check` and `git diff --check` passed. The full `browser-checks.js` suite returned `passed:true` across 220 rounds and 482 crest sources at 320/375/768/1440px with no browser errors or external requests, and offline `file:` play kept 10 options with a working clock.
- Two checked-in assertions in `tests/elapsed-time-readout.test.mjs` needed the new helper injected into their `vm.runInNewContext` scope, since `elapsedTimeFrame` now references it; no assertion was weakened or removed.


## Wrapping desktop career grid — 18 September 2026

- The owner asked why some connector arrows were missing in a 16-club career. They were not: `.club:nth-child(4n):after{content:none}` deliberately drops the connector at each row end. Investigating surfaced a real defect instead — ADR 0018's eight-club pages put card 9 visually to the **right** of card 4, so reading straight across the top row (which a left-to-right layout invites) ran `1→2→3→4→9→10→5→6→7→8`, skipping the middle of the career.
- Replaced paging with one flat club list in a plain wrapping grid: six columns on desktop, mobile's four unchanged ([ADR 0022](docs/adr/0022-wrapping-desktop-career-grid.md)). Connector suppression is now per-grid (`4n` mobile, `6n` desktop) rather than one global rule.
- Measured across **all 220 careers at 900/1128/1280/1440px**: **0 visual/chronological order mismatches** and **0 careers requiring horizontal scrolling** (the paged layout needed it for 74, 34%). Row distribution 92/108/19/1 for one/two/three/four rows.
- An intermediate single-uninterrupted-row design was implemented and then rejected on measurement: it pushed **159 of 220 careers (72%)** onto a horizontal scrollbar, including six- and seven-club careers that previously fit, directly contradicting ADR 0016. Careers over 12 clubs instead compact their cards (`.timeline-long`, desktop-scoped), which brought the worst case (Paulo da Silva, 19 clubs) to a first answer option at y≈1023px — **better than the paged layout's y≈1071px** for the same career. Both sit below a 900px fold; that is pre-existing for this career, reproduced against unchanged `origin/main`, and not introduced here.
- Two checked-in browser suites were found to be silently testing the wrong thing and were fixed: `tests/career-arrow-checks.js` and `tests/browser-checks.js` both loaded `index.html?lang=en` without `&unlimited=1`, so after Daily became the default landing mode `render()` short-circuited into `DailyUI` and their Unlimited fixtures were never exercised. Verified pre-existing by reproducing the identical `Hint 1 order/value` failure against unmodified `origin/main` on a separate baseline server (`isDaily:true` on both). `tests/browser-checks.js` additionally computed row counts from `.timeline-page`, which no longer exists.
- Local Node verification passed **128/128** across the arrow-layout, model, daily, SEO, points-FAQ, account-boundaries, guest-session, elapsed-time, social-preview, privacy and leaderboard suites; `tests/source-check.py`, `scripts/generate-daily-schedule.mjs --check` and `git diff --check` passed.
- Real-Chromium verification: the updated `career-arrow-checks.js` returned `passed:true` (19-club career, 4 rows, no scrolling, no clipped labels), and the full `browser-checks.js` suite returned `passed:true` across **220 rounds, 482 crest sources**, layouts at 320/375/768/1440px, with no browser errors and no external requests. Offline `file:` play verified independently (10 options, decoded crests, correct order, no scrolling). Port 4173 was owned by another worktree, so an isolated harness on 4174 with URL substitution only was used, per AGENTS.md.
...
- Hosted migration `202609160003` was applied to Supabase project `derabona` (`iaebecfxjwjzkapqdeha`) after an authenticated dry run listed that migration alone. Exact readback reports **210 players, 247 candidates, 51,657 rivals and 693 memberships**, with zero missing `all` memberships, zero players without rivals and zero orphan rival references; Carlos Lobatón has only the intended `all` membership. The immutable gameplay snapshot remained byte-for-byte identical at **100 results, 103 rounds and 345 receipts**. Migration history is aligned through `202609160003`, and the final dry run reports the database up to date. Static Pages publication is recorded separately after the main-branch release.
