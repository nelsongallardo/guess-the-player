...
- Replaced paging with one flat club list in a plain wrapping grid: six columns on desktop, mobile's four unchanged ([ADR 0021](docs/adr/0021-wrapping-desktop-career-grid.md)). Connector suppression is now per-grid (`4n` mobile, `6n` desktop) rather than one global rule.
