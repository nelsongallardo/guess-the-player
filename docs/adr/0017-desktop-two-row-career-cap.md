...
- Reading order stays strictly chronological in the DOM either way - only the *visual* path changes. On desktop, each card's connector arrow points down to the card directly below it (same column, next chronological club) via `.club:nth-child(odd):not(:last-child):after{content:"↓";...}`, and the existing base "➜" rule (unchanged, still `top:32px;right:-24px`) handles the even/bottom card of each pair pointing right to the next column. So the path reads as chronological pairs: down within a pair, then right to the next pair - not the single unbroken row ADR 0016 (and the original pre-0016 desktop) used.
