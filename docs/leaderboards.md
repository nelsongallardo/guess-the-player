...
All migrations through `202609160003_expand_ranked_roster_160_to_210.sql` are applied to the live database. The 14 September deployment repaired an earlier 60-to-80 roster gap. The 16 September deployment then discovered that later edits to the already-applied baseline export had not changed the hosted database: production was still at 80 players while the pending ninth-batch migration assumed 110. Both initial roster attempts failed transactionally before the migration was recorded. PRs #6 and #7 made that still-unapplied migration self-contained for the real predecessor state, after which the 80-to-120, 120-to-160 and reviewed 160-to-210 migrations applied successfully. Exact hosted readback is recorded in [TESTING.md](../TESTING.md).
...
- `supabase/migrations/202609160001_ninth_roster_expansion.sql`: forward roster synchronization to the reviewed 120-player state. Before its first successful hosted application, it was made self-contained for the actual 80-player predecessor by upserting the complete 120-state candidates, players and memberships before rebuilding rivals.
- `supabase/migrations/202609160002_expand_ranked_roster_120_to_160.sql`: generated forward roster synchronization from the published 120-player ninth batch to the reviewed 160-player model. It preserves results, receipts and active rounds, adds/updates roster and candidate metadata, adds memberships, and rebuilds future rival rows. Local fresh-replay and upgrade tests pass; hosted application and exact readback are recorded in `TESTING.md`.
- `supabase/migrations/202609160003_expand_ranked_roster_160_to_210.sql`: generated forward synchronization to the reviewed 210-player model. It adds the new playable and peer-bank candidates, preserves existing player/candidate metadata and memberships, and rebuilds only future rival pools. The hosted application preserved exact hashes of results, rounds and receipts; final counts and invariants are recorded in `TESTING.md`.
