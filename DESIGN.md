...
## Game contract
- Choose a competition (Champions League, Premier League, La Liga, Argentine Primera División, Brasileirão) or All Players (the default) from a button next to Difficulty, reachable at any time and never forced. The chosen competition persists in the saved game state and survives reload.
...
- Difficulty is Easy/Medium/Hard, default Medium. Rank eligible distractors (the competition-scoped pool where one applies, otherwise the full pool) by `12 × shared unique clubs + 5 × same country + 4 × overlapping broad role + 3 × career-year intersection/union + 2/(1 + unique-club-count difference)`. Reserve B/C sides share their parent only for similarity scoring, not chronology or ambiguity exclusions. Active `present` endpoints use the research snapshot year.
