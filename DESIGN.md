...
## Product
A football-career guessing game in one portable `index.html`: no build, framework, account, API key, external fonts, or runtime network dependency. Forty curated players, split equally between European and South American national teams. Data verification precedes app implementation; supporting research lives alongside (not required to play).
...
## Game contract
- Shuffle a 40-player deck without replacement; an explicit end-of-deck recap and play-again action follows round 40.
...
## Verification matrix
1. Data: exactly 40 unique player IDs/names; continent count 20/20; sources for every entry; all ordered club names mapped to valid crest bytes; no national-team nodes; every pool has four or more distinct incorrect names excluding the answer.
...
6. Deck: all forty distinct rounds before recap; replay resets session stats; rapid clicks do not skip rounds.
