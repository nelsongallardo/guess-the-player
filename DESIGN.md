...
- Retain all tighter-origin matches if there are fewer than four; fill remaining slots from the best `8 − retained matches` candidates in the boundary tier. If four or more same-system rivals exist, only use that system. Shuffle before ranking for ties and shuffle final positions. Sparse systems and old eras need further roster research; no algorithm can guarantee every round is equally hard.
