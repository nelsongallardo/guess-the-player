async page => {
  ...
  try {
    ...
    ok(await file.evaluate(()=>state.finished&&CareerGame.stats(state).score===4000&&CareerGame.stats(state).streak===40),'All 40 offline rounds complete');
    ...
    return {passed:true,protocol:'file:',networkOffline:true,rounds:seen.size,uniqueCrestSources:crests.size,score:4000,fileSaveReload:true,storageDeniedPlayable:true,errors,networkRequests:requests};
