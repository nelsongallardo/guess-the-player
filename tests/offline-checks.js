async page => {
  ...
  try {
    ...
    for(let i=0;i<60;i++){
      ...
      if(i===0){for(let h=0;h<2;h++)await file.locator('#hint').click();ok(await file.locator('#hint').isDisabled(),'Offline hints work');}
    ...
    return {passed:true,protocol:'file:',networkOffline:true,rounds:seen.size,uniqueCrestSources:crests.size,score:5960,fileSaveReload:true,storageDeniedPlayable:true,errors,networkRequests:requests};
