async page => {
  ...
  try {
    ...
    const total=await file.evaluate(()=>PLAYERS.length);
    for(let i=0;i<total;i++){
      ...
      const decoded=await file.evaluate(async()=>{const careerImages=[...document.querySelectorAll('#timeline img')];await Promise.all(careerImages.map(image=>image.decode()));return careerImages.length>0&&careerImages.every(image=>image.naturalWidth>0&&!image.hidden);});ok(decoded,'Offline crest decoding');
    ...
    // Round 0 used all three available hints (100 x 0.4 = 40); every
    // remaining first-try, no-hint win scores 100.
    const expectedScore=40+(total-1)*100;
    ok(await file.evaluate(({expectedScore,total})=>state.finished&&CareerGame.stats(state).score===expectedScore&&CareerGame.stats(state).streak===total,{expectedScore,total}),`All ${total} offline rounds complete`);
    ...
    return {passed:true,protocol:'file:',networkOffline:true,rounds:seen.size,uniqueCrestSources:crests.size,score:expectedScore,fileSaveReload:true,storageDeniedPlayable:true,errors,networkRequests:requests};
