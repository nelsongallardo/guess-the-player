async page => {
  ...
  try {
    ...
    ok(await file.evaluate(()=>state.finished&&CareerGame.stats(state).score===5000&&CareerGame.stats(state).streak===50),'All 50 offline rounds complete');
    ...
    return {passed:true,protocol:'file:',networkOffline:true,rounds:seen.size,uniqueCrestSources:crests.size,score:5000,fileSaveReload:true,storageDeniedPlayable:true,errors,networkRequests:requests};
