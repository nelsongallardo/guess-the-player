async page => {
  ...
  try {
    ...
    ok(await file.evaluate(()=>state.finished&&CareerGame.stats(state).score===6000&&CareerGame.stats(state).streak===60),'All 60 offline rounds complete');
    ...
    return {passed:true,protocol:'file:',networkOffline:true,rounds:seen.size,uniqueCrestSources:crests.size,score:6000,fileSaveReload:true,storageDeniedPlayable:true,errors,networkRequests:requests};
