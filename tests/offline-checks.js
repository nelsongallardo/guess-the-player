async page => {
  ...
  try {
    ...
    ok(await file.evaluate(()=>state.finished&&CareerGame.stats(state).score===3000&&CareerGame.stats(state).streak===30),'All 30 offline rounds complete');
    ...
    return {passed:true,protocol:'file:',networkOffline:true,rounds:seen.size,uniqueCrestSources:crests.size,score:3000,fileSaveReload:true,storageDeniedPlayable:true,errors,networkRequests:requests};
