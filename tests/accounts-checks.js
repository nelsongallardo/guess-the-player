...
async page => {
  ...
  try{
    ...
    await p.waitForFunction(()=>{const m=document.querySelector('#elapsed-time');return m&&!m.hidden&&!m.classList.contains('is-frozen')&&document.querySelector('#elapsed-time-value').textContent!=='0:00';});
    ok(true,'Ranked elapsed time counts up from the server startedAt clock');
    ...
    await p.waitForFunction(()=>document.querySelector('.elapsed-time').classList.contains('is-frozen'));
    ok(true,'Ranked elapsed time freezes once the round resolves');
    await p.locator('#elapsed-time').click();
    ok((await p.locator('#elapsed-time-note').textContent()).includes('3 hints used')&&(await p.locator('#elapsed-time-note').textContent()).includes('73 points'),'Ranked elapsed time explains the actual server hint count and points on click');
    await p.locator('#elapsed-time').click();
