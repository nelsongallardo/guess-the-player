async page => {
  ...
  await page.evaluate(()=>{sessionStorage.clear();localStorage.removeItem('touchline.career.v1');});await page.reload();
  ...
  for(const language of ['en','es']){
    ...
    const seen=new Set();const total=await page.evaluate(()=>PLAYERS.length);
    ...
    ok(seen.size===total,'Full unique deck');
    ok(await page.evaluate(total=>state.finished&&CareerGame.stats(state).score===total*100,total),'Full playthrough score');
    // Finishing the deck marks every player "seen" (ADR 0005), which would
    ...
    await page.evaluate(()=>{seen.clear();saveSeen();});
  ...
  // Replay is a recap action. Complete the current test deck through the pure
  // model so this assertion exercises the real visible Replay control without
  // adding another slow button-by-button roster playthrough.
  await page.evaluate(()=>{while(!state.finished){CareerGame.answer(state,CareerGame.playerAt(state).name,0);CareerGame.next(state);}render(false,true);});
  await page.locator('#replay').click();
  ...
  ok(await page.evaluate(()=>loadHistory().length===0&&state.difficulty==='hard'&&state.competition==='all'&&CareerGame.stats(state).score===0&&lifetime.score===0&&lifetime.streak===0&&GuestStorage.getItem('touchline.lifetime.v1')===null),'Reset clears results, the lifetime score/streak, and starts automatic Hard');
  ...
  try{
    await isolated.addInitScript(()=>{for(const name of ['localStorage','sessionStorage'])Object.defineProperty(window,name,{get(){throw new Error('Storage denied');}});});
