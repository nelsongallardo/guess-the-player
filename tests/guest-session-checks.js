async page => {
  ...
  try{
    ...
    await context.route('**/index.html*',async route=>{const response=await route.fetch();const html=(await response.text()).replace(/Object\.freeze\(\{url:'[^']*',anonKey:'[^']*'\}\)/,"Object.freeze({url:'',anonKey:''})");await route.fulfill({response,body:html});});await fresh.reload();await fresh.locator('#account-open').click();ok(await fresh.locator('#google-login').isDisabled(),'Unconfigured Google is honestly disabled');await fresh.locator('#account-close').click();
