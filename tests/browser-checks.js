async page => {
  ...
  await choose(first.name);ok((await view()).stats.score===100,'Third-attempt win awards 100');
  ...
  const lost=await view();ok(lost.result==='lost'&&lost.stats.score===100&&lost.stats.streak===0,'Three misses reset streak but retain points');
  ...
  for(let i=0;i<30;i++){
    if(i===29)await page.setViewportSize({width:375,height:667});
  ...
  ok(await page.locator('#review li').count()===30,'All 30 result entries');ok((await page.locator('#summary-caption').textContent()).includes(`Best streak: ${best}`),'Best streak recap');
  ...
  checks.push('all 30 rounds and crest loads, mixed win/loss outcomes, recap, final-save reload, replay');
