...
function ranked(){
  const nodes=new Map(),node=id=>{if(!nodes.has(id))nodes.set(id,{value:'all',dataset:{},replaceChildren(){},classList:{toggle(){}},querySelector(){return null;},focus(){},scrollIntoView(){}});return nodes.get(id);};
  const c={Accounts:{configured:true,hasStoredSession:()=>true,session:{user:{id:'A'}},request:()=>new Promise(()=>{})},AuthCallback:{},language:'en',$:node,window:{addEventListener(){}},setTimeout:()=>0,crypto:{randomUUID:()=> 'key'},GuestStorage:{clear(){}},seen:new Set(),defaultLifetime:()=>({}),CareerGame:{create:()=>({})},resetRoundClock(){},applyLanguage(){},render(){},PLAYERS:[]};vm.createContext(c);
