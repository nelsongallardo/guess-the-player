async page => {
 ...
 const run=async(p,url,language)=>{
  ...
  const history=await p.evaluate(()=>localStorage.getItem('touchline.history.v1'));ok(history&&JSON.parse(history).length>0,'Real completed history seeded');
  ...
  ok(await p.evaluate(()=>localStorage.getItem('touchline.history.v1'))===history,'Earned history unchanged');
