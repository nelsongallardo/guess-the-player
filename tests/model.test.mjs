...
test('exactly 90 explicit researched records: 45 Europe + 45 South America', () => {
  assert.equal(players.length,90);
  assert.equal(new Set(players.map(p=>p.name)).size,90);
  assert.equal(new Set(players.map(p=>p.id)).size,90);
  for (const continent of ['Europe','South America']) assert.equal(players.filter(p=>p.continent===continent).length,45);
...
test('all 90 rounds: first/second/third-attempt wins, losses, score, streak and recap',()=>{
  ...
  assert.equal(s.finished,true);assert.equal(new Set(s.deck).size,90);assert.equal(g.validate(s),true);
...
test('persistence accepts every legitimate state and rejects malformed or impossible saves',()=>{
  ...
  for(const bad of [null,{},42,'x',[],{...clone(good),finished:'true'},{...clone(good),version:2},{...clone(good),roundIndex:90},{...clone(good),roundIndex:-1},{...clone(good),roundIndex:.5},{...clone(good),finished:true}])assert.equal(g.validate(bad),false);
  ...
  for(let i=0;i<90;i++){g.answer(good,g.playerAt(good).name);assert.equal(g.validate(clone(good)),true);g.next(good);assert.equal(g.validate(clone(good)),true);}
  assert.equal(g.stats(good).score,9000);assert.equal(g.stats(good).streak,90);
});
...
test('shuffle does not mutate inputs and decks never repeat a player',()=>{
  ...
  const starts=new Set();for(let i=0;i<200;i++){const s=g.create();assert.equal(new Set(s.deck).size,90);starts.add(s.deck[0]);assert.equal(g.validate(s),true);}
...
test('difficulty changes preserve progress and never reshuffle a started round',()=>{
  ...
  for(const level of ['easy','medium','hard']){
    ...
    for(let i=0;i<90;i++){assert.equal(g.roundAt(game).difficulty,level);g.answer(game,g.playerAt(game).name);g.next(game);assert.equal(g.validate(clone(game)),true);}
    assert.equal(game.finished,true);assert.equal(g.stats(game).score,9000);
  }
});
...
test('published 30-player saves survive roster expansion and finish their original deck',()=>{
  ...
  const fresh=g.create(saved.difficulty);assert.equal(fresh.deck.length,90);assert.equal(fresh.difficulty,'hard');
...
test('create(difficulty, competitionId) restricts the deck to that competition, with a safe fallback for unknown ids',()=>{
  ...
  const all=g.create('medium','all');assert.equal(all.competition,'all');assert.equal(all.deck.length,90);
  const bogus=g.create('medium','not-a-real-competition');assert.equal(bogus.competition,'all');assert.equal(bogus.deck.length,90);
  const omitted=g.create('medium');assert.equal(omitted.competition,'all');assert.equal(omitted.deck.length,90);
});
...
test('Spanish copy, all 90 career notes and every country/position are translated',()=>{
  ...
  assert.equal(Object.keys(SPANISH_NOTES).length,90);
  for(const p of players){
  ...
  // earlier pre-expansion count; only the footer reflects the current 90/45/45 roster.
  for(const lang of ['en','es']){assert.ok(COPY[lang].footer.includes('90 '));assert.ok(COPY[lang].footer.includes('45 '));assert.ok(COPY[lang].rules.includes('70 '));}
