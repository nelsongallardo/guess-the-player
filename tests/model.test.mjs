...
test('exactly 120 explicit researched records: 60 Europe + 60 South America', () => {
  assert.equal(players.length,120);
  assert.equal(new Set(players.map(p=>p.name)).size,120);
  assert.equal(new Set(players.map(p=>p.id)).size,120);
  for (const continent of ['Europe','South America']) assert.equal(players.filter(p=>p.continent===continent).length,60);
  ...
  for(const p of players){
    assert.ok(['2026-09-11','2026-09-12','2026-09-14','2026-09-15'].includes(p.verifiedAt)); assert.ok(p.country && p.position && p.clubs.length);
...
test('all 120 rounds: first/second/third-attempt wins, losses, score, streak and recap',()=>{
  ...
  assert.equal(s.finished,true);assert.equal(new Set(s.deck).size,120);assert.equal(g.validate(s),true);
...
test('shuffle does not mutate inputs and decks never repeat a player',()=>{
  ...
  const starts=new Set();for(let i=0;i<200;i++){const s=g.create();assert.equal(new Set(s.deck).size,120);starts.add(s.deck[0]);assert.equal(g.validate(s),true);}
...
test('difficulty changes preserve progress and never reshuffle a started round',()=>{
  ...
  for(const level of ['easy','medium','hard']){
    ...
    for(let i=0;i<120;i++){assert.equal(g.roundAt(game).difficulty,level);g.answer(game,g.playerAt(game).name);g.next(game);assert.equal(g.validate(clone(game)),true);}
    assert.equal(game.finished,true);assert.equal(g.stats(game).score,12000);
  }
});
...
test('published 30-player saves survive roster expansion and finish their original deck',()=>{
  ...
  const fresh=g.create(saved.difficulty);assert.equal(fresh.deck.length,120);assert.equal(fresh.difficulty,'hard');
...
test('published 40-player saves preserve guesses, hints and order through expansion',()=>{
  ...
  const fresh=g.create(saved.difficulty);assert.equal(fresh.deck.length,120);assert.equal(fresh.difficulty,'hard');
  const additions=players.filter(p=>!original.deck.includes(p.id));assert.equal(additions.length,80);
...
test('published 50-player saves finish their original deck after the third expansion',()=>{
  ...
  const fresh=g.create(saved.difficulty);assert.equal(fresh.deck.length,120);assert.equal(fresh.difficulty,'hard');
  const additions=players.filter(p=>!original.deck.includes(p.id));assert.equal(additions.length,70);
...
test('create(difficulty, competitionId) restricts the deck to that competition, with a safe fallback for unknown ids',()=>{
  ...
  const all=g.create('medium','all');assert.equal(all.competition,'all');assert.equal(all.deck.length,120);
  const bogus=g.create('medium','not-a-real-competition');assert.equal(bogus.competition,'all');assert.equal(bogus.deck.length,120);
  const omitted=g.create('medium');assert.equal(omitted.competition,'all');assert.equal(omitted.deck.length,120);
});
...
test('Spanish copy, all 120 career notes and every country/position are translated',()=>{
  ...
  assert.equal(Object.keys(SPANISH_NOTES).length,120);
  for(const p of players){
  ...
  // The rules dialog and footer both describe the current 120-player roster.
  for(const lang of ['en','es']){assert.ok(COPY[lang].footer.includes('120 '));assert.ok(COPY[lang].footer.includes('60 '));assert.ok(COPY[lang].rules.includes('120 '));}
