...
test('exactly 110 explicit researched records: 55 Europe + 55 South America', () => {
  assert.equal(players.length,110);
  assert.equal(new Set(players.map(p=>p.name)).size,110);
  assert.equal(new Set(players.map(p=>p.id)).size,110);
  for (const continent of ['Europe','South America']) assert.equal(players.filter(p=>p.continent===continent).length,55);
...
test('all 110 rounds: first/second/third-attempt wins, losses, score, streak and recap',()=>{
  ...
  assert.equal(s.finished,true);assert.equal(new Set(s.deck).size,110);assert.equal(g.validate(s),true);
...
test('shuffle does not mutate inputs and decks never repeat a player',()=>{
  ...
  const starts=new Set();for(let i=0;i<200;i++){const s=g.create();assert.equal(new Set(s.deck).size,110);starts.add(s.deck[0]);assert.equal(g.validate(s),true);}
...
test('difficulty changes preserve progress and never reshuffle a started round',()=>{
  ...
  for(const level of ['easy','medium','hard']){
    ...
    for(let i=0;i<110;i++){assert.equal(g.roundAt(game).difficulty,level);g.answer(game,g.playerAt(game).name);g.next(game);assert.equal(g.validate(clone(game)),true);}
    assert.equal(game.finished,true);assert.equal(g.stats(game).score,11000);
  }
});
...
test('published 30-player saves survive roster expansion and finish their original deck',()=>{
  ...
  const fresh=g.create(saved.difficulty);assert.equal(fresh.deck.length,110);assert.equal(fresh.difficulty,'hard');
...
test('create(difficulty, competitionId) restricts the deck to that competition, with a safe fallback for unknown ids',()=>{
  ...
  const all=g.create('medium','all');assert.equal(all.competition,'all');assert.equal(all.deck.length,110);
  const bogus=g.create('medium','not-a-real-competition');assert.equal(bogus.competition,'all');assert.equal(bogus.deck.length,110);
  const omitted=g.create('medium');assert.equal(omitted.competition,'all');assert.equal(omitted.deck.length,110);
});
...
test('Spanish copy, all 110 career notes and every country/position are translated',()=>{
  ...
  assert.equal(Object.keys(SPANISH_NOTES).length,110);
  for(const p of players){
