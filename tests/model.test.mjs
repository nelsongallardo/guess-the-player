...
test('exactly 70 explicit researched records: 35 Europe + 35 South America', () => {
  assert.equal(players.length,70);
  assert.equal(new Set(players.map(p=>p.name)).size,70);
  assert.equal(new Set(players.map(p=>p.id)).size,70);
  for (const continent of ['Europe','South America']) assert.equal(players.filter(p=>p.continent===continent).length,35);
...
test('70,000 option samples always contain ten unique names and exactly one correct answer',()=>{
  for(const p of players){
...
test('all 70 rounds: first/second/third-attempt wins, losses, score, streak and recap',()=>{
  ...
  assert.equal(s.finished,true);assert.equal(new Set(s.deck).size,70);assert.equal(g.validate(s),true);
...
test('persistence accepts every legitimate state and rejects malformed or impossible saves',()=>{
  ...
  for(const bad of [null,{},42,'x',[],{...clone(good),finished:'true'},{...clone(good),version:2},{...clone(good),roundIndex:70},{...clone(good),roundIndex:-1},{...clone(good),roundIndex:.5},{...clone(good),finished:true}])assert.equal(g.validate(bad),false);
  ...
  for(let i=0;i<70;i++){g.answer(good,g.playerAt(good).name);assert.equal(g.validate(clone(good)),true);g.next(good);assert.equal(g.validate(clone(good)),true);}
  assert.equal(g.stats(good).score,7000);assert.equal(g.stats(good).streak,70);
});
...
test('shuffle does not mutate inputs and decks never repeat a player',()=>{
  ...
  const starts=new Set();for(let i=0;i<200;i++){const s=g.create();assert.equal(new Set(s.deck).size,70);starts.add(s.deck[0]);assert.equal(g.validate(s),true);}
...
test('difficulty changes preserve progress and never reshuffle a started round',()=>{
  ...
  for(const level of ['easy','medium','hard']){
    ...
    for(let i=0;i<70;i++){assert.equal(g.roundAt(game).difficulty,level);g.answer(game,g.playerAt(game).name);g.next(game);assert.equal(g.validate(clone(game)),true);}
    assert.equal(game.finished,true);assert.equal(g.stats(game).score,7000);
  }
});
...
test('published 30-player saves survive roster expansion and finish their original deck',()=>{
  ...
  const fresh=g.create(saved.difficulty);assert.equal(fresh.deck.length,70);assert.equal(fresh.difficulty,'hard');
...
test('create(difficulty, competitionId) restricts the deck to that competition, with a safe fallback for unknown ids',()=>{
  ...
  const all=g.create('medium','all');assert.equal(all.competition,'all');assert.equal(all.deck.length,70);
  const bogus=g.create('medium','not-a-real-competition');assert.equal(bogus.competition,'all');assert.equal(bogus.deck.length,70);
  const omitted=g.create('medium');assert.equal(omitted.competition,'all');assert.equal(omitted.deck.length,70);
});
...
test('Spanish copy, all 70 career notes and every country/position are translated',()=>{
  ...
  assert.equal(Object.keys(SPANISH_NOTES).length,70);
  for(const p of players){
  ...
  for(const lang of ['en','es']){assert.ok(COPY[lang].footer.includes('70 '));assert.ok(COPY[lang].footer.includes('35 '));assert.ok(COPY[lang].rules.includes('70 '));}
