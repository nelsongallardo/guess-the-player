...
test('exactly 80 explicit researched records: 40 Europe + 40 South America', () => {
  assert.equal(players.length,80);
  assert.equal(new Set(players.map(p=>p.name)).size,80);
  assert.equal(new Set(players.map(p=>p.id)).size,80);
  for (const continent of ['Europe','South America']) assert.equal(players.filter(p=>p.continent===continent).length,40);
  ...
  for(const p of players){
    assert.ok(['2026-09-11','2026-09-12','2026-09-14'].includes(p.verifiedAt)); assert.ok(p.country && p.position && p.clubs.length);
...
test('80,000 option samples always contain ten unique names and exactly one correct answer',()=>{
  for(const p of players){
...
test('all 80 rounds: first/second/third-attempt wins, losses, score, streak and recap',()=>{
  ...
  assert.equal(s.finished,true);assert.equal(new Set(s.deck).size,80);assert.equal(g.validate(s),true);
...
test('persistence accepts every legitimate state and rejects malformed or impossible saves',()=>{
  ...
  for(const bad of [null,{},42,'x',[],{...clone(good),finished:'true'},{...clone(good),version:2},{...clone(good),roundIndex:80},{...clone(good),roundIndex:-1},{...clone(good),roundIndex:.5},{...clone(good),finished:true}])assert.equal(g.validate(bad),false);
  ...
  for(let i=0;i<80;i++){g.answer(good,g.playerAt(good).name);assert.equal(g.validate(clone(good)),true);g.next(good);assert.equal(g.validate(clone(good)),true);}
  assert.equal(g.stats(good).score,8000);assert.equal(g.stats(good).streak,80);
});
...
test('shuffle does not mutate inputs and decks never repeat a player',()=>{
  ...
  const starts=new Set();for(let i=0;i<200;i++){const s=g.create();assert.equal(new Set(s.deck).size,80);starts.add(s.deck[0]);assert.equal(g.validate(s),true);}
...
test('difficulty changes preserve progress and never reshuffle a started round',()=>{
  ...
  for(const level of ['easy','medium','hard']){
    ...
    for(let i=0;i<80;i++){assert.equal(g.roundAt(game).difficulty,level);g.answer(game,g.playerAt(game).name);g.next(game);assert.equal(g.validate(clone(game)),true);}
    assert.equal(game.finished,true);assert.equal(g.stats(game).score,8000);
  }
});
...
test('published 30-player saves survive roster expansion and finish their original deck',()=>{
  ...
  const fresh=g.create(saved.difficulty);assert.equal(fresh.deck.length,80);assert.equal(fresh.difficulty,'hard');
...
test('published 40-player saves preserve guesses, hints and order through expansion',()=>{
  ...
  const fresh=g.create(saved.difficulty);assert.equal(fresh.deck.length,80);assert.equal(fresh.difficulty,'hard');
  const additions=players.filter(p=>!original.deck.includes(p.id));assert.equal(additions.length,40);
...
test('published 50-player saves finish their original deck after the third expansion',()=>{
  ...
  const fresh=g.create(saved.difficulty);assert.equal(fresh.deck.length,80);assert.equal(fresh.difficulty,'hard');
  const additions=players.filter(p=>!original.deck.includes(p.id));assert.equal(additions.length,30);
...
test('create(difficulty, competitionId) restricts the deck to that competition, with a safe fallback for unknown ids',()=>{
  ...
  const all=g.create('medium','all');assert.equal(all.competition,'all');assert.equal(all.deck.length,80);
  const bogus=g.create('medium','not-a-real-competition');assert.equal(bogus.competition,'all');assert.equal(bogus.deck.length,80);
  const omitted=g.create('medium');assert.equal(omitted.competition,'all');assert.equal(omitted.deck.length,80);
});
...
test('Spanish copy, all 80 career notes and every country/position are translated',()=>{
  ...
  assert.equal(Object.keys(SPANISH_NOTES).length,80);
  for(const p of players){
  ...
  // concurrent scoring/FAQ change owns that text) and still states the
  // pre-expansion count; only the footer reflects the current 80/40/40 roster.
  for(const lang of ['en','es']){assert.ok(COPY[lang].footer.includes('80 '));assert.ok(COPY[lang].footer.includes('40 '));assert.ok(COPY[lang].rules.includes('70 '));}
