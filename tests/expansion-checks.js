async page => {
  ...
  const saves=[__LEGACY_SAVE__,__LEGACY_SAVE_40__],results=[];
  for(const legacy of saves){
    ...
    ok(await page.evaluate(()=>state.deck.length===50&&state.difficulty==='hard'),'Replay upgrades to 50 and preserves level');
    ok(await page.locator('#round-number').textContent()==='01 / 50','New round total');
    ok(await page.locator('#progress').getAttribute('max')==='50','New progress denominator');
    results.push({legacyRoundCount:total,newRoundCount:50,exactSavePreserved:true});
  }
  const additions=await page.evaluate(old=>PLAYERS.filter(p=>!old.deck.includes(p.id)).map(p=>p.id),saves[1]);
  ok(additions.length===10&&additions.includes('javier-saviola')&&additions.includes('claudio-pizarro'),'All ten additional players playable');
  const examples=[];
  await page.setViewportSize({width:375,height:667});
  for(const id of ['javier-saviola','claudio-pizarro']){
  ...
  return {passed:true,results,additions,examples};
}
